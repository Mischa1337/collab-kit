...
describe('recording what happened', () => {
  it('keeps an event without any of the optional parts', async () => {
    ...
    expect(created).toMatchObject({ kind: 'presence', createdBy: 'alice' });
...
describe('reading back', () => {
  async function three(): Promise<void> {
    await recordEvent(storage.db, { kind: 'read', createdBy: 'alice', anchor: on() });
    await recordEvent(storage.db, { kind: 'read', createdBy: 'alice', anchor: on('statement-3') });
    await recordEvent(storage.db, { kind: 'visit', createdBy: 'bob', anchor: on('statement-3') });
  }
  ...
  it('narrows down to one unit', async () => {
    ...
    expect(found.map((event) => event.createdBy).toSorted()).toEqual(['alice', 'bob']);
  });

  it('narrows down by actor and by kind', async () => {
    await three();

    await expect(readEvents(storage.db, { anchor: on(), createdBy: 'bob' })).resolves.toHaveLength(
      1,
    );
  ...
  it('answers newest first and honours a limit', async () => {
    const first = await recordEvent(storage.db, { kind: 'read', createdBy: 'a', anchor: on() });
    const second = await recordEvent(storage.db, { kind: 'read', createdBy: 'b', anchor: on() });
...
describe('the newest of a kind', () => {
  it('is the reading mark, without anything being overwritten', async () => {
    ...
    await recordEvent(storage.db, { kind: 'read', createdBy: 'bob', anchor: on(), at: first });
    await recordEvent(storage.db, { kind: 'read', createdBy: 'bob', anchor: on(), at: second });
    ...
    await expect(
      readEvents(storage.db, { anchor: on(), createdBy: 'bob', kind: 'read' }),
    ).resolves.toHaveLength(2);
  });

  it('does not mix two people up', async () => {
    const hers = new ObjectId();
    await recordEvent(storage.db, { kind: 'read', createdBy: 'alice', anchor: on(), at: hers });
    await recordEvent(storage.db, { kind: 'read', createdBy: 'bob', anchor: on() });

    const mark = await latestEvent(storage.db, { anchor: on(), createdBy: 'alice', kind: 'read' });
    expect(mark?.at).toEqual(hers);
  });

  it('answers with null when nothing of that kind happened', async () => {
    await expect(
      latestEvent(storage.db, { anchor: on(), createdBy: 'dora', kind: 'read' }),
    ).resolves.toBeNull();
  });
});
